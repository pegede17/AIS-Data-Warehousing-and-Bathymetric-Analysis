...
class Boxes(Resource):
    def get(self):
        ...
        FROM {dim_cell} d inner join
            (SELECT max(max_draught) max_draught, MIN(min_draught) min_draught, cell_id, SUM(trajectory_count) count, SUM(avg_draught * trajectory_count)/SUM(trajectory_count) avg_draught
            from {fact_cell}
