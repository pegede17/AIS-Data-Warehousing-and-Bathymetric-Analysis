...
def fill_fact_cell_1000m(date):
    ...
    cur = connection.cursor()
    cur.execute(f"""
    ALTER TABLE fact_cell_3034_1000m DISABLE TRIGGER ALL;
    INSERT INTO fact_cell_3034_1000m(date_id, 
                                    cell_id, 
                                    ship_type_id, 
                                    type_of_mobile_id, 
                                    is_draught_trusted, 
    ...
    SELECT date_start_id,
            ...
            histogram(avg_speed_knots, 0,100,1000)
    from fact_trajectory_sailing traj 
    inner join bridge_traj_sailing_cell_3034_1000m b 
        on traj.trajectory_id = b.trajectory_id
    inner join dim_cell_3034_1000m on dim_cell_3034_1000m.cell_id = b.cell_id
    WHERE date_start_id = {date}
    GROUP BY date_start_id, dim_cell_3034_1000m.cell_id, ship_type_id, type_of_mobile_id, is_draught_trusted;
    ALTER TABLE fact_cell_3034_1000m ENABLE TRIGGER ALL;
    """)
    connection.commit()
    cur.close()
    connection.close()
